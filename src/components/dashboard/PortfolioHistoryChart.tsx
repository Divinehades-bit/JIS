...
import usePortfolioHistoryStore, {
  type PortfolioSnapshot,
} from "../../store/portfolioHistoryStore";
...
const limitChartPoints = (
  ...
  if (snapshots.length <= maximumPoints) {
    return snapshots;
  }
...
const PortfolioHistoryChart = () => {
  ...
  const chartData = useMemo(() => {
    ...
    const chartSnapshots =
      limitChartPoints(dailySnapshots);

    if (chartSnapshots.length === 0) {
      return null;
    }

    const values = chartSnapshots.flatMap(
      (snapshot) => [
    ...
    if (
      ...
      const fallbackPadding = Math.max(
        Math.abs(minimumValue) * 0.05,
        1,
      );

      minimumValue -= fallbackPadding;
      maximumValue += fallbackPadding;
    } else {
      const valuePadding =
      ...
      <div className="flex flex-col gap-4 border-b border-slate-100 px-5 py-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Historical market value recorded
            by JIS after portfolio changes and
            price updates.
          </p>
        </div>
        ...
        <div className="flex min-h-72 items-center justify-center p-8 text-center">
          <div>
            ...
            <p className="mt-2 max-w-sm text-sm text-slate-500">
              Update prices or record a
              transaction to create the first
              historical snapshot.
            </p>
          </div>
        </div>
                  ...
                  <div className="flex items-center justify-between gap-5">
                    ...
                    <span className="font-semibold text-slate-700">
                      {hoveredPoint.snapshot
                        ...
                        ? "+"
                        : ""}
                      {hoveredPoint.snapshot.totalReturn.toFixed(
                        2,
                      )}
                      %
                    </span>
